...
def calcular_arbitraje():
    ...
    try:
        ...
        if not isinstance(buy_methods, list):
            print(f"[ROBUSTEZ] buy_methods no es lista: {type(buy_methods)}")
            buy_methods = []
        if not isinstance(sell_methods, list):
            print(f"[ROBUSTEZ] sell_methods no es lista: {type(sell_methods)}")
            sell_methods = []
        
        # Convertir elementos a diccionarios si son strings
